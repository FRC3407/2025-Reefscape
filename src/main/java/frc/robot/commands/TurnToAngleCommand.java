...
public class TurnToAngleCommand extends Command {
  ...
  public void execute() {
    ...
    double speed = 0.2 * Math.signum(angleDifference);
    m_DriveSubsystem.drive(0, 0, speed, false);
  ...
  public boolean isFinished() {
    double angleDifference = goalAngle - m_DriveSubsystem.getHeading();
    if (m_TargetAngle > 0 && angleDifference <= 0) {
      return true;
    }
    else if (m_TargetAngle < 0 && angleDifference >= 0){
      return true;
    }
    return false;
